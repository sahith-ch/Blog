import React from 'react'
import Styles  from "./Card.module.css"
import Image from 'next/image'
import { url } from 'inspector'
import Link from 'next/link'
interface Props {
    title:string,
    description:string,
    author:string,
    image:string
}

function Card(props: Props) {
    const {title,description,author,image} = props
console.log("props",props)
    return (
        <div className={Styles.container}>
<Link href={`/users/:id`}>

<p className='flex flex-row  font-bold  hover:underline '>

<Image className={Styles.image} alt=" "  height={200} width={200} src={'data:image/webp;base64,'}/> 
<span>{author}</span>    
</p>    
</Link>
<div className='flex flex-col md:flex-row md:align-start md:justify-start align-center justify-center'>
<div className={Styles.thumbnail}>
    {image?
    <img  alt=" " width={200} height={200}  src={image}/>:<>no</>}
</div>
<div className={Styles.details}>

<div dangerouslySetInnerHTML={{__html:title}} className={Styles.title}></div>
<p className='text-ellipsis overflow-hidden line-clamp-2 w-full'>{description}</p>
</div>

</div>

        </div>
    )
}

export default Card
